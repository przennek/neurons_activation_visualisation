...
Mimo krótkiego czasu treningu taki obraz klasyfikowany jest przez sieć VGG19 jako ważka z pewnością sięgającą 96\%. I dotyczy to wszystkich testowanych przezemnie klas. VGG rozpoznaje przedmioty na podstawie prawdopodobieństwa wystąpienia pewnej kombinacji filtrów - niestety samo nie nadaje się do generowania realistycznych obrazów danej klasy.

Wiem, że lepsze efekty udało się osiągnąć z niezwykle głębokimi sieciami typu incepcja i spróbuję je odtworzyć w dalszej części tej pracy. 

Warte uwagi są też \textit{ang. Generative Adversarial Networks (GANs)} przy ich pomocy można generować obrazy, które są wstanie oszukać człowieka. // TODO być może warto będzie napisać o GANach zamiast czegoś.
